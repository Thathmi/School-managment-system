...
if (empty($u)) { // check email is empty
    ...
    echo "Please enter your verification code";
       ...
       // serach from database whether there is a admin with given username and erification code
    $rs = Database::search("SELECT * FROM `admin` WHERE `email`='".$u."' AND `code`='".$vc."'");
    if($rs->num_rows==1){

           // update password in user with given email in database
         Database::iud("UPDATE `admin` SET `password`='".$np."' WHERE `email`='".$u."'");
         echo "1";

    }else{//there is no user with given email nor incorect verification code
