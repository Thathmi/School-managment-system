...
if (empty($f)) { //check first name is empty
    ...
    $s1 = Database::search("SELECT * FROM `reg_email` WHERE `email`='" . $e . "'");
    if ($s1->num_rows == 1) {
        $r1 = $s1->fetch_assoc();
        ...
        if ($u == $un) { //is types username is equals to the usename in database
            ...
            // insert the records to ac_officer table and set the status as non- verified until user logging for the 1st time
            $s = Database::iud("INSERT INTO `ac_officer`(`fname`,`lname`,`email`,`mobile`,`password`,`uname`,`gender_id`,`status_id`,`code`) VALUES('" . $f . "','" . $l . "','" . $e . "','" . $m . "','" . $p . "','" . $u . "','" . $g . "','2','" . $c . "')");
            echo "1";
        } else {
