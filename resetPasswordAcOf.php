...
if (empty($u)) { // check username is empty
    ...
    // serach from database whether there is a officer with given username and erification code
    $rs = Database::search("SELECT * FROM `ac_officer` WHERE `uname`='".$u."' AND `code`='".$vc."'");
    if($rs->num_rows==1){ // there is an officer

        // update password in user with given username in database 
         Database::iud("UPDATE `ac_officer` SET `password`='".$np."' WHERE `uname`='".$u."'");
         echo "1";
